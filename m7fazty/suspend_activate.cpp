...
void suspend_activate::on_Activate_pushButton_clicked()
{
    ...
    if(username.empty())
        QMessageBox::warning(this,"Activate Account","Empty Fields Not Allowed");
    else
    {
        if(sign_up::users_read[username]==NULL)
            QMessageBox::warning(this,"Activate Account","User Not Found");
        else
        {
...
void suspend_activate::on_suspend_pushButton_clicked()
{
    string username=ui->username_lineEdit->text().toStdString();
    if(username.empty())
        QMessageBox::warning(this,"Suspend Account","Empty Fields Not Allowed");
    else
    {
        if(sign_up::users_read[username]==NULL)
